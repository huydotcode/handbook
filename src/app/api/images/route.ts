...
export async function POST(request: Request) {
  ...
  try {
    ...
    return new Response(JSON.stringify(imagesUrl), { status: 200 });
  } catch (error) {
    return (
      new Response("Failed to upload image on Cloudinary"), { status: 500 }
    );
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    for (const img of images) {
      if (!img) {
        return new Response(JSON.stringify({ message: "Image invalid" }), {
          status: 400,
        });
      }
